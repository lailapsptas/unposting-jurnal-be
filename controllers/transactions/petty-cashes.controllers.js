...
export class PettyCashesControllers {
  ...
  static async createOrUpdatePettyCashes(req, res) {
    try {
      const result =
        await PettyCashesControllers.pettyCashesService.createOrUpdatePettyCashes(
          req.body
        );
      res.status(200).json(result);
    } catch (error) {
  ...
  static async approvePettyCash(req, res) {
    try {
      const result =
        await PettyCashesControllers.pettyCashesService.approvedPettyCash(
          req.params.id,
          req.body
        );
